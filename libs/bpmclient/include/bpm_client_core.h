 ...
 * parameters are the service to connect to, the service_board to get
 * the board_type, the parameters of the BPM, and the number of samples to 
 * acquire before and after the trigger. */
 ...
 * parameters are the service to connect to, the service_board to get
 * the board_type, the parameters of the BPM, and the number of samples to 
 * acquire before and after the trigger. This variation allows specifying 
 ...
 * parameters are the service to connect to, the service_board to get
 * the board_type, the parameters of the BPM, and the number of samples 
 * to acquire before and after the trigger. */
bpm_single_pass_t *bpm_single_pass_new_log_mode (char *broker_endp, int verbose,
        ...
        char *service_board, bpm_parameters_t *bpm_parameters, uint32_t samples_pre,
        uint32_t samples_post, uint32_t num_shots);

/* Create an instance of the single-pass acquisition client, with the log
 * filemode specified * by "log_mode" as in fopen () call, and the send/recv
 * timeout in ms. The single-pass parameters are the service to connect to, 
 * the service_board to get the board_type, the parameters of the BPM, and 
 * the number of samples to acquire before and after the trigger. */
