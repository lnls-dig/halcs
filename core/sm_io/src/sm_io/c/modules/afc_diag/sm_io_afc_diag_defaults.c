...
#define SMIO_AFC_DIAG_LIBHALCSCLIENT_LOG_MODE                "a"
       ...
       const char *log_file_name)
{
    ...
    halcs_client_t *config_client = halcs_client_new_log_mode (broker_endp, 0,
            log_file_name, SMIO_AFC_DIAG_LIBHALCSCLIENT_LOG_MODE);
    ASSERT_ALLOC(config_client, err_alloc_client);

    uint32_t card_slot = 0;
    client_err = halcs_get_afc_diag_card_slot (config_client, service, &card_slot);
    ASSERT_TEST(client_err == HALCS_CLIENT_SUCCESS, "Could not get AFC_DIAG card slot",
            err_param_get, SMIO_ERR_CONFIG_DFLT);

    uint32_t ipmi_addr = 0;
    client_err = halcs_get_afc_diag_ipmi_addr (config_client, service, &ipmi_addr);
    ASSERT_TEST(client_err == HALCS_CLIENT_SUCCESS, "Could not get AFC_DIAG IPMI addr",
            err_param_get, SMIO_ERR_CONFIG_DFLT);
