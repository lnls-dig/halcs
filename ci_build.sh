...
if [ "$PURE_MAKE" = yes ]; then
    ...
    build-wrapper-linux-x86-64 --out-dir bw-output mkdir -p build && cd build && cmake -Dhalcs_BOARD_OPT="$BOARD" ../ && make VERBOSE=1 && make DESTDIR="${BUILD_PREFIX}" install
elif [ "$CPACK" = yes ]; then
    for board in ${CPACK_BOARDS}; do
        build-wrapper-linux-x86-64 --out-dir bw-output mkdir -p build && cd build && cmake -Dhalcs_BOARD_OPT="$board" ../ && cpack -G "${CPACK_GENERATORS}"
    done
else
    build-wrapper-linux-x86-64 --out-dir bw-output mkdir -p build && cd build && cmake -Dhalcs_BOARD_OPT="$BOARD" ../ && make VERBOSE=1 && make DESTDIR="${BUILD_PREFIX}" install
fi
