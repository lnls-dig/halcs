...
/********************** ORBIT INTLK Functions ********************/

/* Orbit interlock global enable function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_en (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_en);
halcs_client_err_e halcs_get_orbit_intlk_en (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_en);

/* Orbit interlock global cleaer function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_clr (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_clr);
halcs_client_err_e halcs_get_orbit_intlk_clr (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_clr);

/* Orbit interlock minium sum enable function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_min_sum_en (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_min_sum_en);
halcs_client_err_e halcs_get_orbit_intlk_min_sum_en (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_min_sum_en);

/* Orbit interlock translation enable function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_trans_en (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_trans_en);
halcs_client_err_e halcs_get_orbit_intlk_trans_en (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_en);

/* Orbit interlock translation clear function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_trans_clr (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_trans_clr);
halcs_client_err_e halcs_get_orbit_intlk_trans_clr (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_clr);

/* Orbit interlock angular enable function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_ang_en (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_ang_en);
halcs_client_err_e halcs_get_orbit_intlk_ang_en (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_en);

/* Orbit interlock angular clear function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_ang_clr (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_ang_clr);
halcs_client_err_e halcs_get_orbit_intlk_ang_clr (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_clr);

/* Orbit interlock translation bigger X function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_trans_bigger_x (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_bigger_x);

/* Orbit interlock translation bigger Y. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_trans_bigger_y (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_bigger_y);

/* Orbit interlock translation bigger X latched function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_trans_bigger_ltc_x (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_bigger_ltc_x);

/* Orbit interlock translation bigger Y ltached function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_trans_bigger_ltc_y (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_bigger_ltc_y);

/* Orbit interlock translation any trip function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_trans_bigger_any (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_bigger_any);

/* Orbit interlock translation trip function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_trans_bigger (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_bigger);

/* Orbit interlock translation trip latched function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_trans_bigger_ltc (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_trans_bigger_ltc);

/* Orbit interlock angular bigger X function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_ang_bigger_x (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_bigger_x);

/* Orbit interlock angular bigger Y function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_ang_bigger_y (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_bigger_y);

/* Orbit interlock angular bigger X latched function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_ang_bigger_ltc_x (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_bigger_ltc_x);

/* Orbit interlock angular bigger Y ltached function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_ang_bigger_ltc_y (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_bigger_ltc_y);

/* Orbit interlock angular any trip function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_ang_bigger_any (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_bigger_any);

/* Orbit interlock angular trip function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_ang_bigger (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_bigger);

/* Orbit interlock angular trip latched function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_orbit_intlk_ang_bigger_ltc (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_ang_bigger_ltc);
 ...
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_min_sum (halcs_client_t *self, char *service,
        uint32_t orbit_intlk_min_sum);
halcs_client_err_e halcs_get_orbit_intlk_min_sum (halcs_client_t *self, char *service,
        uint32_t *orbit_intlk_min_sum);

/* Orbit interlock translation max X threshold function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_trans_max_x (halcs_client_t *self, char *service,
        int32_t orbit_intlk_trans_max_x);
halcs_client_err_e halcs_get_orbit_intlk_trans_max_x (halcs_client_t *self, char *service,
        int32_t *orbit_intlk_trans_max_x);

/* Orbit interlock translation max Y threshold function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_trans_max_y (halcs_client_t *self, char *service,
        int32_t orbit_intlk_trans_max_y);
halcs_client_err_e halcs_get_orbit_intlk_trans_max_y (halcs_client_t *self, char *service,
        int32_t *orbit_intlk_trans_max_y);

/* Orbit interlock angular max X threshold function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_ang_max_x (halcs_client_t *self, char *service,
        int32_t orbit_intlk_ang_max_x);
halcs_client_err_e halcs_get_orbit_intlk_ang_max_x (halcs_client_t *self, char *service,
        int32_t *orbit_intlk_ang_max_x);

/* Orbit interlock angular max Y threshold function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_orbit_intlk_ang_max_y (halcs_client_t *self, char *service,
        int32_t orbit_intlk_ang_max_y);
...
halcs_client_err_e halcs_set_orbit_intlk_trans_min_x (halcs_client_t *self, char *service,
        int32_t orbit_intlk_trans_min_x);
...
halcs_client_err_e halcs_set_orbit_intlk_trans_min_y (halcs_client_t *self, char *service,
        int32_t orbit_intlk_trans_min_y);
 ...
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_act_part (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_act_part);
halcs_client_err_e halcs_get_fofb_ctrl_act_part (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_act_part);

/* FOFB Controller err_clr function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_err_clr (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_err_clr);
halcs_client_err_e halcs_get_fofb_ctrl_err_clr (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_err_clr);

/* FOFB Controller cc_enable function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_cc_enable (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_cc_enable);
halcs_client_err_e halcs_get_fofb_ctrl_cc_enable (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_cc_enable);

/* FOFB Controller tfs_override function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_tfs_override (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_tfs_override);
halcs_client_err_e halcs_get_fofb_ctrl_tfs_override (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_tfs_override);

/* FOFB Controller bpm_id function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_bpm_id (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_bpm_id);
halcs_client_err_e halcs_get_fofb_ctrl_bpm_id (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_bpm_id);

/* FOFB Controller time_frame_len function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_time_frame_len (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_time_frame_len);
halcs_client_err_e halcs_get_fofb_ctrl_time_frame_len (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_time_frame_len);

/* FOFB Controller mgt_powerdown function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_mgt_powerdown (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_mgt_powerdown);
halcs_client_err_e halcs_get_fofb_ctrl_mgt_powerdown (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_mgt_powerdown);

/* FOFB Controller mgt_loopback function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_mgt_loopback (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_mgt_loopback);
halcs_client_err_e halcs_get_fofb_ctrl_mgt_loopback (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_mgt_loopback);

/* FOFB Controller time_frame_dly function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_time_frame_dly (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_time_frame_dly);
halcs_client_err_e halcs_get_fofb_ctrl_time_frame_dly (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_time_frame_dly);

/* FOFB Controller golden_orb_x function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_golden_orb_x (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_golden_orb_x);
halcs_client_err_e halcs_get_fofb_ctrl_golden_orb_x (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_golden_orb_x);

/* FOFB Controller golden_orb_y function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_golden_orb_y (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_golden_orb_y);
halcs_client_err_e halcs_get_fofb_ctrl_golden_orb_y (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_golden_orb_y);

/* FOFB Controller cust_feature function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_cust_feature (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_cust_feature);
halcs_client_err_e halcs_get_fofb_ctrl_cust_feature (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_cust_feature);

/* FOFB Controller rx_polarity function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_rx_polarity (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_rx_polarity);
halcs_client_err_e halcs_get_fofb_ctrl_rx_polarity (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_rx_polarity);

/* FOFB Controller payloadsel function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_payloadsel (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_payloadsel);
halcs_client_err_e halcs_get_fofb_ctrl_payloadsel (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_payloadsel);

/* FOFB Controller fofbdatasel function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_set_fofb_ctrl_fofbdatasel (halcs_client_t *self, char *service,
        uint32_t fofb_ctrl_fofbdatasel);
halcs_client_err_e halcs_get_fofb_ctrl_fofbdatasel (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_fofbdatasel);

/* Status only functions */

/* FOFB Controller firmware_ver function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_firmware_ver (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_firmware_ver);

/* FOFB Controller sys_status function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
 ...
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_link_up (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_link_up);

/* FOFB Controller time_frame_count function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
 ...
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_fod_process_time (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_fod_process_time);

/* FOFB Controller bpm_count function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_bpm_count (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_bpm_count);

/* FOFB Controller bpm_id_rdback function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_bpm_id_rdback (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_bpm_id_rdback);

/* FOFB Controller tf_length_rdback function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_tf_length_rdback (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_tf_length_rdback);

/* FOFB Controller powerdown_rdback function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_powerdown_rdback (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_powerdown_rdback);

/* FOFB Controller loopback_rdback function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
halcs_client_err_e halcs_get_fofb_ctrl_loopback_rdback (halcs_client_t *self, char *service,
        uint32_t *fofb_ctrl_loopback_rdback);

/* FOFB Controller faival_rdback function. The functions returns
 * HALCS_CLIENT_SUCCESS if the parameter was correctly set or error
 * (see halcs_client_err.h for all possible errors)*/
...
halcs_client_err_e halcs_get_rtmlamp_ohwr_pi_kp (halcs_client_t *self, char *service,
        uint32_t chan, uint32_t *pi_kp);

halcs_client_err_e halcs_set_rtmlamp_ohwr_pi_ti (halcs_client_t *self, char *service,
        uint32_t chan, uint32_t pi_ti);
halcs_client_err_e halcs_get_rtmlamp_ohwr_pi_ti (halcs_client_t *self, char *service,
        uint32_t chan, uint32_t *pi_ti);

halcs_client_err_e halcs_set_rtmlamp_ohwr_pi_sp (halcs_client_t *self, char *service,
...
halcs_client_err_e halcs_set_fofb_processing_ram_write_enable (halcs_client_t *self, char *service,
        uint32_t fofb_processing_ram_write_enable);
halcs_client_err_e halcs_get_fofb_processing_ram_write_enable (halcs_client_t *self, char *service,
        uint32_t *fofb_processing_ram_write_enable);

halcs_client_err_e halcs_set_fofb_processing_ram_addr_val (halcs_client_t *self, char *service,
        uint32_t fofb_processing_ram_addr_val);
halcs_client_err_e halcs_get_fofb_processing_ram_addr_val (halcs_client_t *self, char *service,
        uint32_t *fofb_processing_ram_addr_val);

halcs_client_err_e halcs_set_fofb_processing_ram_data_in_val (halcs_client_t *self, char *service,
        uint32_t fofb_processing_ram_data_in_val);
halcs_client_err_e halcs_get_fofb_processing_ram_data_in_val (halcs_client_t *self, char *service,
        uint32_t *fofb_processing_ram_data_in_val);

halcs_client_err_e halcs_set_fofb_processing_ram_data_out_val (halcs_client_t *self, char *service,
        uint32_t fofb_processing_ram_data_out_val);
halcs_client_err_e halcs_get_fofb_processing_ram_data_out_val (halcs_client_t *self, char *service,
        uint32_t *fofb_processing_ram_data_out_val);
