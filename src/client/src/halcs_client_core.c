...
halcs_client_err_e halcs_fofb_processing_coeffs_ram_bank_read(
    ...
    ASSERT_TEST(err == HALCS_CLIENT_SUCCESS,
        "[halcs_fofb_processing_coeffs_ram_bank_read] Something wrong happened",
        coeffs_ram_bank_read, HALCS_CLIENT_ERR_AGAIN);

    /* If we are here, then the request was successfully processed */
    DBE_DEBUG(DBG_LIB_CLIENT | DBG_LVL_TRACE,
        "[halcs_fofb_processing_coeffs_ram_bank_read] Coefficients"
        "successfully read\n");

coeffs_ram_bank_read:
    return err;
}
...
halcs_client_err_e halcs_fofb_processing_coeffs_ram_bank_write(
    ...
    input[0] = chan;
    memcpy(&input[1], coeffs.data, FOFB_PROCESSING_DATA_BLOCK_MAX_PARAMS*sizeof(uint32_t));
    ...
    /* Check if any error occurred */
    ASSERT_TEST(err == HALCS_CLIENT_SUCCESS,
        "fofb_processing_coeffs_ram_bank_write: Something wrong happened",
         coeffs_ram_bank_write, HALCS_CLIENT_ERR_AGAIN);

    /* If we are here, then the request was successfully processed */
    DBE_DEBUG(DBG_LIB_CLIENT | DBG_LVL_TRACE,
        "[halcs_fofb_processing_coeffs_ram_bank_write] Coefficients"
        "successfully written\n");
    
coeffs_ram_bank_write:
    return err;
}

PARAM_FUNC_CLIENT_READ(fofb_processing_coeffs_fixed_point_pos)
{
    return param_client_read (self, service, FOFB_PROCESSING_OPCODE_SET_GET_COEFFS_FIXED_POINT_POS, fofb_processing_coeffs_fixed_point_pos);
}
