...
smio_fmc130m_4ch_t * smio_fmc130m_4ch_new (smio_t *parent)
{
    ...
    ASSERT_TEST(err == SMIO_SUCCESS, "Could not get board_type",
            err_get_board_type);
    DBE_DEBUG (DBG_SM_IO | DBG_LVL_INFO, "[sm_io:fmc130m_4ch_core] board_type: %s, Inst ID: %u\n",
            board_type, inst_id);

    /* Lookup symbol */
    const uint32_t **num_fmc130m_4ch_smios_p =
        hutils_lookup_symbol ("pvar_const_uint32_t_p_", board_type, "_num_fmc130m_4ch_smios");
    ASSERT_ALLOC(num_fmc130m_4ch_smios_p, err_lookup_sym);
    const uint32_t num_fmc130m_4ch_smios = **num_fmc130m_4ch_smios_p;
    DBE_DEBUG (DBG_SM_IO | DBG_LVL_INFO, "[sm_io:fmc130m_4ch_core] num_fmc130m_4ch_smios: %u, "
            "Inst ID = %u\n", num_fmc130m_4ch_smios, inst_id);

    const uint32_t **fmc130m_4ch_pca9547_addr = 
        hutils_lookup_symbol ("pvar_const_uint32_t_p_", board_type, "_fmc130m_4ch_pca9547_addr");
    ASSERT_ALLOC(fmc130m_4ch_pca9547_addr, err_lookup_sym);
    DBE_DEBUG (DBG_SM_IO | DBG_LVL_INFO, "[sm_io:fmc130m_4ch_core] PCA9547 address, "
            " addr: 0x%08X, Inst ID: %u\n", (*fmc130m_4ch_pca9547_addr)[inst_id],
            inst_id);
    const uint32_t **fmc130m_4ch_24aa64_addr = 
        hutils_lookup_symbol ("pvar_const_uint32_t_p_", board_type, "_fmc130m_4ch_24aa64_addr");
    ASSERT_ALLOC(fmc130m_4ch_24aa64_addr, err_lookup_sym);
    DBE_DEBUG (DBG_SM_IO | DBG_LVL_INFO, "[sm_io:fmc130m_4ch_core] 24AA64 address, "
            " addr: 0x%02X, Inst ID: %u\n", 
            (*fmc130m_4ch_24aa64_addr)[inst_id], inst_id);
